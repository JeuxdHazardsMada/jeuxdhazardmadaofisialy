...
-- Storage policies for chat-files (bucket created separately)
...
CREATE POLICY "chat_files_delete_own" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'chat-files' AND ((storage.foldername(name))[1] = auth.uid()::text OR public.has_role(auth.uid(), 'admin')));

-- ============ NEW FEATURES ============

-- Premium bonuses: admin-granted temporary premium (1-5 days)
...
-- Voice calls (persistent + global notifications)
